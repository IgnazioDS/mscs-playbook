...
from app.worker import IN_PROGRESS_STATES, _connect_postgres, _row_to_state, _utc_now
...
def _row_to_replay_job(row: tuple | None) -> ReplayJob | None:
    ...
    return ReplayJob(
        ...
        total_events=row[14],
        published_events=row[15],
        completed_events=row[16],
        failed_events=row[17],
        last_error=row[18],
    )
...
def _claim_next_replay_job(pg_conn, now: datetime, lease_timeout) -> ReplayJob | None:
    with pg_conn.cursor() as cur:
        cur.execute(
            """
        ...
        cur.execute(
            ...
            RETURNING
                ...
                last_error
            """,
            (now, now, now + lease_timeout, candidate[0]),
        )
...
def _materialize_replay_job_events(pg_conn, job: ReplayJob, now: datetime) -> list[ReplayJobEvent]:
    ...
    _update_replay_job_counts(pg_conn, job.replay_job_id, now)
    return _load_replay_job_events(pg_conn, job.replay_job_id)
...
def _update_replay_job_counts(pg_conn, replay_job_id: str, now: datetime) -> ReplayJob:
    with pg_conn.cursor() as cur:
        cur.execute(
            """
            SELECT
        ...
        total_events, published_events, completed_events, failed_events = cur.fetchone()
        cur.execute(
            """
            UPDATE replay_jobs
            SET total_events = %s,
...
def _heartbeat_replay_job(pg_conn, replay_job_id: str, now: datetime, lease_timeout) -> None:
    with pg_conn.cursor() as cur:
        cur.execute(
            """
            UPDATE replay_jobs
            ...
            WHERE replay_job_id = %s
              AND status = 'running'
            """,
            (now, now + lease_timeout, replay_job_id),
        )
...
def _finalize_replay_job(
    ...
    *,
    ...
    with pg_conn.cursor() as cur:
        ...
        finalized = _row_to_replay_job(cur.fetchone())

    action = f"{job.job_type}_{'completed' if status == 'completed' else 'failed'}"
    _record_audit(
        pg_conn,
        action=action,
        actor="replay-runner",
...
def process_replay_job(
    ...
    while True:
        now = _utc_now()
        _heartbeat_replay_job(pg_conn, job.replay_job_id, now, settings.replay_lease_timeout)
        job = _update_replay_job_counts(pg_conn, job.replay_job_id, now)
        ...
        job = _update_replay_job_counts(pg_conn, job.replay_job_id, _utc_now())
        events = _load_replay_job_events(pg_conn, job.replay_job_id)
...
def main() -> None:
    ...
    logger = configure_json_logger("replay-runner", settings.log_level)
    ...
    logger.info(
        ...
        extra={"component": "replay-runner", "processing_state": "startup"},
    )

    try:
        while True:
            job = _claim_next_replay_job(pg_conn, _utc_now(), settings.replay_lease_timeout)
            if job is None:
                time.sleep(settings.replay_poll_seconds)
                continue
