...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Local AI mini-project CLI")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "route-plan":
        print(run_route_planning(seed=args.seed, size=args.size, density=args.density, out=args.out))
        return 0
    if args.command == "schedule":
        print(run_schedule(seed=args.seed, out=args.out))
        return 0
    if args.command == "diagnose":
        print(run_diagnosis(seed=args.seed, out=args.out))
        return 0
