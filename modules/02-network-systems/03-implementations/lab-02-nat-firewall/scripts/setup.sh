...
# Build a three-namespace topology: client -> gateway -> server.
NS_CLIENT="ns-client"
NS_SERVER="ns-server"
NS_GW="ns-gw"
...
# Create two veth pairs: client<->gateway and server<->gateway.
...
# Route client and server traffic through the gateway namespace.
sudo ip -n "$NS_CLIENT" route add default via 10.0.1.1
sudo ip -n "$NS_SERVER" route add default via 10.0.2.1

# Enable IP forwarding in the gateway so it can route packets.
sudo ip netns exec "$NS_GW" sysctl -w net.ipv4.ip_forward=1 >/dev/null

# Initialize firewall: drop forwarding by default, allow established flows.
...
# Start a simple HTTP server in the server namespace.
